...
RSpec.describe DataFetchers::GitHub, :vcr do
  ...
  describe '#call' do
    ...
    let(:result_pr_event) { service_result[:events].find { |event| event[:type] == 'pr' } }
    let(:result_review_event) { service_result[:events].find { |event| event[:type] == 'review' } }
    let(:result_comment_event) { service_result[:events].find { |event| event[:type] == 'comment' } }
    ...
    it 'reveals pr details' do
      expect(result_pr_event).to eq({ author: 'petrokoriakin', type: 'pr',
                                      created_at: Time.zone.parse('2021-04-27 16:45:32') })
    end

    it 'reveals review details' do
      expect(result_review_event).to eq({ author: 'petrokoriakin', type: 'review',
                                          created_at: Time.zone.parse('2021-04-27 16:38:21') })
    end

    it 'reveals comment details' do
      expect(result_comment_event).to eq({ author: 'petrokoriakin', type: 'comment',
